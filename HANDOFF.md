...
## 2026-09-08 总览与卡片表单
总览新增今天消费免息天数前三名，复用 /cards/ranking，与完整榜单共享排序。排行榜独立加载、重试，支持空状态和跨时区说明。
银行/卡组织默认直接填写自定义名称，通过箭头或方向键展开常用选项。菜单相对输入框整体定位，避免同一行字段高度影响；已有自定义名称也可展开全部快捷选项。
本轮未部署。验证结果见 docs/verification.md 对应章节。
