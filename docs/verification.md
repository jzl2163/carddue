...
- 卡片详情提供“复制为新卡”，添加页面提供已有卡片来源选择（包含归档卡）。来源从当前账户的鉴权卡片列表读取。
- 以独立状态快照预填 CardInput；名称加“（副本）”并限制长度，尾号清空。复用银行、卡组织、地区、时区、币种、识别色、日期规则、年费和备注。保存沿用创建接口，新卡具有独立身份；不复制账期金额、还款历史、事项或卡片专属通知规则。适用于所有卡的通知规则仍按已有行为覆盖新卡。
- 切换来源会确认是否覆盖尚未保存的表单；无效来源显示错误并允许从空白开始。无后端或数据库结构变更。
- 在 Oracle Docker 执行类型检查、静态构建和资源预算检查；本轮未进行浏览器交互测试或真实消息测试，无本机依赖安装。

- GitHub 同步因自动审批拒绝而暂停：仓库仍为 public，用户此前要求私有化。此次源码仅保留 Oracle 工作目录与独立构建目录；部署基于 d894c67 加本次本地修改，未创建 GitHub 提交。
