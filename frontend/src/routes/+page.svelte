<script lang="ts">
  ...
  import { dateLabel, kindLabels } from '$lib/utils';
  import type { Card, Dashboard, Upcoming } from '$lib/types';
  import Loading from '$lib/components/Loading.svelte';
  import Empty from '$lib/components/Empty.svelte';
  import Button from '$lib/components/ui/button/Button.svelte';
  import RankingPreview from '$lib/components/RankingPreview.svelte';
  let data=$state<Dashboard|null>(null),cards=$state<Card[]>([]),error=$state(''),busy=$state('');
  const today=$derived(data?.today || '');
  const due=$derived(data?.events.filter(e=>e.kind==='payment_due'&&!e.paid).sort((a,b)=>a.days_until-b.days_until) || []);
  const next=$derived(due.find(e=>data&&e.days_until>=0));
  const overdue=$derived(due.filter(e=>data&&e.days_until<0));
  const upcoming=$derived(data?.events.filter(e=>e.days_until>=0&&!e.paid).slice(0,8)||[]);
...
<div class="stack">
<div class="grid-3"><div class="panel stat"><div class="stat-label">正在管理<CreditCard size={16}/></div><div class="stat-number">{cards.filter(c=>c.active).length}<small> 张</small></div><div class="stat-caption">有效信用卡</div></div><div class="panel stat"><div class="stat-label">未来 30 天<Clock3 size={16}/></div><div class="stat-number">{due.filter(e=>e.days_until>=0&&e.days_until<=30).length}<small> 笔</small></div><div class="stat-caption">待处理还款日</div></div><div class="panel stat"><div class="stat-label">已完成<CircleCheck size={16}/></div><div class="stat-number">{data.events.filter(e=>e.kind==='payment_due'&&e.paid&&e.date.slice(0,7)===e.local_today.slice(0,7)).length}<small> 笔</small></div><div class="stat-caption">本月手工标记已还款</div></div></div>
...
<div class="grid-2"><section class="panel hero"><div class="eyebrow">Next payment</div>{#if next}<small>下一个还款日</small><h2>{next.card_name}</h2><p>{dateLabel(next.date)}{#if next.timezone!==data.timezone}<small> · {next.timezone}</small>{/if}</p><div class="hero-bottom"><div><span class="number">{next.days_until}</span><small> 天后到期</small></div><Button disabled={busy===next.id} onclick={()=>paid(next)}>标记已还款<CircleCheck size={16}/></Button></div>{:else}<h2>暂时没有待处理还款</h2><p>添加信用卡后，你的下一个重要日期会显示在这里。</p><div class="hero-bottom"><a class="button" href="/cards/new">添加第一张卡<Plus size={16}/></a></div>{/if}</section>
<section class="panel"><div class="panel-heading"><h2>即将到来</h2><a class="subtle-link" href="/calendar">订阅日历<ArrowUpRight size={13} class="inline"/></a></div>{#each upcoming.slice(0,4) as e}<div class="event-row"><div class="event-main"><div class="date-tile">{Number(e.date.slice(8))}<small>{Number(e.date.slice(5,7))} 月</small></div><div><h3>{e.card_name}</h3><p>{kindLabels[e.kind]||e.title}</p></div></div><span class="badge" class:accent={e.kind==='payment_due'}>{e.days_until===0?'今天':`${e.days_until} 天后`}</span></div>{:else}<Empty title="日程清晰，暂时无事" description="账单、还款和权益事项会自动汇总在这里。"/>{/each}</section></div>
<RankingPreview/>
