...
async fn main() -> anyhow::Result<()> {
    ...
    if command.as_deref() == Some("openapi") {
        println!(
            "{}",
            serde_json::to_string_pretty(&carddue::spec::document())?
        );
        return Ok(());
    }
    if command.as_deref() == Some("generate-key") {
        println!("{}", carddue::crypto::random_token());
        return Ok(());
    }
    tracing_subscriber::fmt()
        ...
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "carddue=info,tower_http=warn,sqlx=warn".into()),
        )
        .init();
    let cfg = Config::load()?;
    let options =
        sqlx::postgres::PgConnectOptions::from_str(&cfg.database_url)?.disable_statement_logging();
    ...
    if cfg.role == "worker" {
        ...
        axum::serve(
            listener,
            carddue::web::router(state.clone())
