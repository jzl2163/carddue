use crate::{
    AppState, auth, calendar, cards, crypto,
    error::{AppError, Result},
    notifications,
};
use axum::{
    Json, Router,
    extract::{DefaultBodyLimit, FromRequest, MatchedPath, Request, State},
        ...
        let Json(value) = Json::<T>::from_request(req, state)
            ...
            .map_err(|rejection| {
                AppError::new(
                    rejection.status(),
                    "VALIDATION_ERROR",
                    "Invalid JSON request; review field names, body size and value types",
                )
            })?;
        Ok(Self(value))
    }
...
async fn ready(State(s): State<AppState>) -> Response {
    match tokio::time::timeout(
        ...
        sqlx::query_scalar::<_, i32>("SELECT 1").fetch_one(&s.db),
    )
    .await
    {
...
async fn openapi(_auth: auth::Auth) -> Json<Value> {
    Json(crate::spec::document())
}
async fn system(State(s): State<AppState>, auth: auth::Auth) -> Result<Json<Value>> {
    ...
    let pending: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM notification_jobs WHERE status IN ('pending','retry','processing')",
    )
    .fetch_one(&s.db)
    .await?;
    let rows = sqlx::query("SELECT role,last_success_at FROM worker_heartbeats ORDER BY role")
        .fetch_all(&s.db)
        .await?;
    ...
    Ok(Json(
        json!({"version":env!("CARGO_PKG_VERSION"),"role":s.config.role,"base_url":s.config.base_url,"secure_cookies":s.config.secure_cookie,"registration_allowed":s.config.allow_registration,"webpush_configured":s.config.vapid_private.is_some(),"pending_jobs":pending,"worker_heartbeats":heartbeats,"private_destination_exceptions":s.config.private_hosts,"insecure_notifications_allowed":s.config.allow_insecure_notifications}),
    ))
}
...
pub async fn security(State(s): State<AppState>, req: Request, next: Next) -> Response {
    ...
    let rejection = if unsafe_method && path.starts_with("/api/") {
        ...
        if origin != Some(s.config.base_url.as_str()) {
            Some(AppError::new(
                StatusCode::FORBIDDEN,
                "ORIGIN_REJECTED",
                "Use exactly one Origin header matching the configured application origin",
            ))
        } else if ![
            "/api/v1/auth/login",
            "/api/v1/auth/setup",
        ...
        .contains(&path.as_str())
        {
            ...
            if token.is_empty()
                || !crypto::equal(csrf, &crypto::sign(&s.config.signing_key, "csrf", &token))
            {
                Some(AppError::new(
                    StatusCode::FORBIDDEN,
                    "CSRF_REJECTED",
    ...
    if path.starts_with("/api/") && response.status().is_client_error() && !json_response {
        ...
        response =
            AppError::new(status, code, "Request path, method or body is invalid").into_response();
    }
    let id = response
        .headers()
        .get("x-request-id")
...
pub fn router(s: AppState) -> Router {
    let api = Router::new()
        ...
        .route("/account", patch(auth::account))
        .route("/account/password", post(auth::password))
        .route("/dashboard", get(cards::dashboard))
        ...
        .route(
            "/cards/{card}/cycles/{cycle}/pay",
            post(cards::pay).delete(cards::unpay),
        )
        .route(
            "/milestones",
            get(cards::milestones).post(cards::create_milestone),
        )
        .route(
            "/milestones/{id}",
            patch(cards::update_milestone).delete(cards::delete_milestone),
        )
        .route(
            "/calendar/feeds",
        ...
        .route(
            "/calendar/feeds/{id}",
            patch(calendar::update).delete(calendar::delete),
        )
        .route("/calendar/feeds/{id}/rotate-token", post(calendar::rotate))
        .route(
            "/notification/connections",
            get(notifications::connections).post(notifications::create_connection),
        )
        .route(
            "/notification/connections/{id}",
            patch(notifications::update_connection).delete(notifications::delete_connection),
        )
        .route(
            "/notification/connections/{id}/test",
            post(notifications::test_connection),
        )
        .route("/notification/webpush/key", get(notifications::vapid))
        .route(
            "/notification/templates",
            get(notifications::templates).post(notifications::create_template),
        )
        .route(
            "/notification/templates/{id}",
            patch(notifications::update_template).delete(notifications::delete_template),
        )
        .route(
        ...
        .route(
            "/notification/rules",
            get(notifications::rules).post(notifications::create_rule),
        )
        .route(
            "/notification/rules/{id}",
            patch(notifications::update_rule).delete(notifications::delete_rule),
        )
        .route("/notification/deliveries", get(notifications::history))
        .route(
        ...
        .route("/settings/system", get(system))
        .route("/settings/audit", get(audit_log))
        .route("/openapi.json", get(openapi))
        .fallback(not_found);
    let fallback = ServeDir::new(&s.config.frontend_dir).fallback(ServeFile::new(format!(
        "{}/200.html",
        s.config.frontend_dir
    )));
    Router::new()
        .nest("/api/v1", api)
        .nest(
            "/cal",
            Router::new()
                .route("/v1/{id}/{token}", get(calendar::subscribe))
                .fallback(not_found),
        )
        ...
        .layer(middleware::from_fn_with_state(s.clone(), security))
        .with_state(s)
}
#[cfg(test)]
mod tests {
