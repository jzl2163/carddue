...
fn secret(name: &str) -> anyhow::Result<Option<String>> {
    match (value(name), value(&format!("{name}_FILE"))) {
        ...
        (None, Some(path)) => Ok(Some(fs::read_to_string(path).with_context(|| format!("Cannot read {name}_FILE"))?.trim().to_owned())),
        _ => Ok(None),
    }
}
fn key(name: &str) -> anyhow::Result<[u8; 32]> {
    let s = secret(name)?.with_context(|| format!("Missing {name}"))?;
    let bytes = URL_SAFE_NO_PAD.decode(s).with_context(|| format!("{name} must be base64url without padding"))?;
    bytes.try_into().map_err(|_| anyhow::anyhow!("{name} must encode exactly 32 random bytes"))
}
impl Config {
    pub fn load() -> anyhow::Result<Self> {
        let u = Url::parse(&value("APP_BASE_URL").unwrap_or_else(|| "https://carddue.example.com".into()))?;
        ...
        if u.scheme() != "https" && !(development && u.scheme() == "http" && matches!(u.host_str(), Some("localhost" | "127.0.0.1" | "[::1]"))) {
            bail!("HTTPS is required; loopback HTTP is permitted only in development");
        }
        let role = value("APP_ROLE").unwrap_or_else(|| "all".into());
        if !["all", "web", "worker"].contains(&role.as_str()) { bail!("Invalid APP_ROLE"); }
        let tz = value("DEFAULT_TIMEZONE").unwrap_or_else(|| "Asia/Shanghai".into());
        tz.parse::<chrono_tz::Tz>().context("Invalid DEFAULT_TIMEZONE")?;
        let setup_token = secret("SETUP_TOKEN")?.context("SETUP_TOKEN is required")?;
        if setup_token.len() < 32 { bail!("SETUP_TOKEN must contain at least 32 random characters"); }
        let vapid_private = secret("VAPID_PRIVATE_KEY")?;
        if let Some(k) = &vapid_private {
            web_push::VapidSignatureBuilder::from_base64_no_sub(k).context("Invalid VAPID_PRIVATE_KEY")?;
        }
        let vapid_subject = value("VAPID_SUBJECT").unwrap_or_else(|| "mailto:admin@example.com".into());
        if !vapid_subject.starts_with("mailto:") && !vapid_subject.starts_with("https://") { bail!("Invalid VAPID_SUBJECT"); }
        Ok(Self {
            database_url: secret("DATABASE_URL")?.context("DATABASE_URL is required")?,
            base_url: u.as_str().trim_end_matches('/').into(),
            bind: value("APP_BIND").unwrap_or_else(|| "0.0.0.0:8080".into()).parse()?,
            role,
            ...
            private_hosts: value("NOTIFICATION_PRIVATE_HOSTS").unwrap_or_default().split(',').map(|s| s.trim().to_ascii_lowercase()).filter(|s| !s.is_empty()).collect(),
            allow_insecure_notifications: value("ALLOW_INSECURE_NOTIFICATIONS").as_deref() == Some("true"),
            vapid_private,
            vapid_subject,
    ...
    pub fn cookie_name(&self) -> &'static str {
        if self.secure_cookie { "__Host-carddue" } else { "carddue" }
    }
}
