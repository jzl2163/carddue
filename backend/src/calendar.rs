use crate::{
    ...
    model::{CardInput, Event, FeedInput},
    web::ApiJson,
};
use axum::{
    Json,
...
pub fn url(s: &AppState, id: Uuid, version: i32) -> String {
    format!(
        ...
        s.config.base_url,
        id,
        crypto::sign(&s.config.signing_key, "feed", &format!("{id}:{version}"))
    )
}
...
async fn save(s: AppState, auth: Auth, id: Option<Uuid>, f: FeedInput) -> Result<Json<Value>> {
    ...
    let version: i32 = if id.is_some() {
        ...
        let count: i64 = sqlx::query_scalar("SELECT count(*) FROM calendar_feeds WHERE user_id=$1")
            .bind(auth.user.id)
            .fetch_one(&mut *tx)
            .await?;
        if count >= 20 {
            return Err(AppError::bad("Calendar feed limit reached"));
        }
        sqlx::query("INSERT INTO calendar_feeds(id,user_id,data) VALUES($1,$2,$3)")
            .bind(key)
            .bind(auth.user.id)
            .bind(json!(f))
            .execute(&mut *tx)
...
pub async fn create(
    ...
    auth: Auth,
    ApiJson(f): ApiJson<FeedInput>,
) -> Result<Json<Value>> {
    save(s, auth, None, f).await
}
pub async fn update(
    State(s): State<AppState>,
    auth: Auth,
...
pub async fn delete(
    ...
    let mut tx = s.db.begin().await?;
    lock_user(&mut tx, auth.user.id).await?;
    let result = sqlx::query("DELETE FROM calendar_feeds WHERE id=$1 AND user_id=$2")
        .bind(id)
        .bind(auth.user.id)
        .execute(&mut *tx)
        .await?;
...
pub fn render(name: &str, events: &[Published]) -> String {
    ...
    for e in events {
        ...
        line(
            &mut out,
            &format!("DTSTAMP:{}", e.modified.format("%Y%m%dT%H%M%SZ")),
        );
        line(
            &mut out,
            &format!("LAST-MODIFIED:{}", e.modified.format("%Y%m%dT%H%M%SZ")),
        );
        line(&mut out, &format!("SEQUENCE:{}", e.sequence));
        line(
            &mut out,
            &format!("DTSTART;VALUE=DATE:{}", e.date.format("%Y%m%d")),
        );
        line(
            &mut out,
            &format!(
...
pub fn included(feed: &FeedInput, event: &Event) -> bool {
    event.active
        && feed.kinds.contains(&event.kind)
        && (feed.card_ids.is_empty() || feed.card_ids.contains(&event.card_id))
        && !(feed.hide_paid && event.paid)
}
...
pub async fn subscribe(
    ...
    let initial = sqlx::query("SELECT user_id,token_version FROM calendar_feeds WHERE id=$1")
        .bind(id)
        .fetch_optional(&s.db)
        .await?
    ...
    let timezone: String = sqlx::query_scalar("SELECT timezone FROM users WHERE id=$1 FOR SHARE")
        .bind(user)
        .fetch_one(&mut *tx)
        .await?;
    ...
    let rows = sqlx::query("SELECT id,data FROM cards WHERE user_id=$1")
        .bind(user)
        .fetch_all(&mut *tx)
        .await?;
    ...
    for r in rows {
        cards.insert(
            r.try_get("id")?,
            serde_json::from_value(r.try_get("data")?)?,
        );
    }
    ...
    for e in events.iter().filter(|e| included(&feed, e)) {
        ...
        let alarms = if e.paid {
            Vec::new()
        } else {
            feed.alarms_days_before.clone()
        };
        let title = title(&feed, e, card);
        let hash = crypto::hash(
            &json!({"date":e.event_date,"title":title,"alarms":alarms,"cancelled":false})
                .to_string(),
        );
        ...
        published.push(Published {
            ...
            alarms,
            sequence: 0,
            modified: feed_updated,
        });
    }
    let previous = sqlx::query(
        "SELECT event_id,last_date,cancelled FROM calendar_publications WHERE feed_id=$1",
    )
    .bind(id)
    .fetch_all(&mut *tx)
    .await?;
    for old in previous {
        ...
        if last_date < start {
            sqlx::query("DELETE FROM calendar_publications WHERE feed_id=$1 AND event_id=$2")
                .bind(id)
                .bind(event_id)
                .execute(&mut *tx)
                .await?;
        ...
        published.push(Published {
            id: event_id,
            date: last_date,
            title: "CardDue event removed".into(),
            cancelled: true,
            alarms: Vec::new(),
            sequence: 0,
            modified: feed_updated,
    ...
    let states = sqlx::query(
        "SELECT event_id,sequence,modified_at FROM calendar_publications WHERE feed_id=$1",
    )
    .bind(id)
    .fetch_all(&mut *tx)
    .await?;
    ...
    for state in states {
        versions.insert(
            state.try_get("event_id")?,
            (state.try_get("sequence")?, state.try_get("modified_at")?),
        );
    }
    for event in &mut published {
        if let Some((sequence, modified)) = versions.get(&event.id) {
    ...
    let not_modified = headers
        ...
        .is_some_and(|v| {
            v.split(',')
                .any(|part| part.trim().trim_start_matches("W/") == etag || part.trim() == "*")
        });
    ...
    response.headers_mut().insert(
        "last-modified",
        HeaderValue::from_str(&httpdate::fmt_http_date(modified.into()))
...
mod tests {
    ...
    fn unicode_folding_and_escape() {
        let mut out = String::new();
        line(&mut out, &format!("SUMMARY:{}", "💳中文".repeat(40)));
        for l in out.split("\r\n") {
            assert!(l.len() <= 75);
        }
        assert_eq!(
            out.replace("\r\n ", "").trim_end(),
            format!("SUMMARY:{}", "💳中文".repeat(40))
        );
        assert_eq!(
            escape("a,b;c\\d\r\nEND:VEVENT"),
            "a\\,b\\;c\\\\d\\nEND:VEVENT"
    ...
    fn all_day_event_and_stable_uid() {
        ...
        changed.date = dates::day(2028, 3, 1);
        changed.sequence += 1;
