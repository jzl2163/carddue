...
pub fn encrypt(key: &[u8; 32], aad: &str, plain: &[u8]) -> Result<String> {
    ...
    let encrypted = cipher.encrypt(XNonce::from_slice(&nonce), Payload { msg: plain, aad: aad.as_bytes() }).map_err(|_| AppError::internal())?;
    let mut bytes = nonce.to_vec();
    bytes.extend(encrypted);
    Ok(format!("v1.{}", URL_SAFE_NO_PAD.encode(bytes)))
}
pub fn decrypt(key: &[u8; 32], aad: &str, encrypted: &str) -> Result<Vec<u8>> {
    let encoded = encrypted.strip_prefix("v1.").ok_or_else(AppError::internal)?;
    let bytes = URL_SAFE_NO_PAD.decode(encoded).map_err(|_| AppError::internal())?;
    if bytes.len() < 40 { return Err(AppError::internal()); }
    XChaCha20Poly1305::new(key.into()).decrypt(XNonce::from_slice(&bytes[..24]), Payload { msg: &bytes[24..], aad: aad.as_bytes() }).map_err(|_| AppError::internal())
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn authenticated_encryption_rejects_substitution() {
        ...
        assert_eq!(decrypt(&key, "owner/connection", &encrypted).unwrap(), b"private");
        assert!(decrypt(&key, "other/connection", &encrypted).is_err());
        assert!(decrypt(&[8; 32], "owner/connection", &encrypted).is_err());
        assert_ne!(encrypted, encrypt(&key, "owner/connection", b"private").unwrap());
    }
    #[test]
