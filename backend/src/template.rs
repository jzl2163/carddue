...
impl Write for LimitedWriter {
    fn write(&mut self, b: &[u8]) -> io::Result<usize> {
        if self.buf.len() + b.len() > self.max {
            return Err(io::Error::other("rendered output too large"));
        }
...
fn render_one(source: &str, context: &Value, html: bool, max: usize) -> Result<String> {
    ...
    env.add_template("message", source)
        .map_err(|e| AppError::bad(format!("Template syntax: {e}")))?;
    let template = env
    ...
    template
        ...
        .map_err(|e| AppError::bad(format!("Template rendering: {}", e.kind())))?;
...
pub fn validate(t: &TemplateInput, base: &str) -> Result<()> {
    text(&t.name, 1, 100)?;
    for value in [
        &t.title, &t.body, &t.html, &t.url, &t.group, &t.sound, &t.level,
    ] {
        text(value, 0, 16000)?;
    }
...
pub fn render(t: &TemplateInput, context: &Value, base: &str) -> Result<Rendered> {
    ...
    let html = ammonia::Builder::default()
        .url_schemes(["https", "mailto"].into_iter().collect())
        .clean(&raw_html)
        .to_string();
    let url = render_one(&t.url, context, false, 2000)?;
    if !url.is_empty() {
        let u = url::Url::parse(&url)
            .map_err(|_| AppError::bad("Notification link must be an absolute URL"))?;
        let b = url::Url::parse(base).map_err(|_| AppError::internal())?;
        if u.origin() != b.origin() || !u.username().is_empty() || u.password().is_some() {
            return Err(AppError::bad(
                "Notification links must use the application origin",
            ));
        }
    }
    let level = render_one(&t.level, context, false, 32)?;
    if !["", "active", "passive", "timeSensitive"].contains(&level.as_str()) {
        return Err(AppError::bad(
            "Unsupported Bark level; critical alerts are not enabled",
        ));
    }
    Ok(Rendered {
        title,
        body: render_one(&t.body, context, false, 12000)?,
        html,
        url,
        group: render_one(&t.group, context, false, 100)?,
        sound: render_one(&t.sound, context, false, 64)?,
        level,
    })
}
#[cfg(test)]
