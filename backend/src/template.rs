...
fn render_one(source: &str, context: &Value, html: bool, max: usize) -> Result<String> {
    ...
    let t = env.get_template("message").map_err(|_| AppError::internal())?;
    let mut output = LimitedWriter { buf: Vec::new(), max };
    t.render_to_write(context, &mut output).map_err(|e| AppError::bad(format!("Template rendering: {}", e.kind())))?;
...
pub fn validate(t: &TemplateInput, base: &str) -> Result<()> {
    text(&t.name,1,100)?;
    for s in [&t.title, &t.body, &t.html, &t.url, &t.group, &t.sound, &t.level] { text(s,0,16000)?; }
...
pub fn render(t: &TemplateInput, context: &Value, base: &str) -> Result<Rendered> {
    ...
    let raw_html = render_one(&t.html, context, true, 64_000)?;
    ...
    Ok(Rendered { title, body: render_one(&t.body, context, false, 12000)?, html, url,
        group: render_one(&t.group, context, false, 100)?, sound: render_one(&t.sound, context, false, 64)?, level })
}
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn errors_and_output_limits() {
        let mut t = TemplateInput::default();
        t.title = "{{ unknown }}".into();
