...
pub async fn plan_all(s: AppState) {
    ...
    loop {
        interval.tick().await;
        if let Err(e) = planning_tick(&s).await {
            tracing::error!(code = e.code, "planner tick failed");
        }
    }
}
async fn planning_tick(s: &AppState) -> Result<()> {
    let users: Vec<Uuid> = sqlx::query_scalar("SELECT id FROM users ORDER BY id")
        .fetch_all(&s.db)
        .await?;
    for user in users {
        let mut tx = s.db.begin().await?;
        let tz: Option<String> =
            sqlx::query_scalar("SELECT timezone FROM users WHERE id=$1 FOR UPDATE SKIP LOCKED")
                .bind(user)
    ...
    sqlx::query("DELETE FROM rate_limits WHERE bucket<$1")
        .bind(Utc::now().timestamp().div_euclid(60) - 5)
        .execute(&s.db)
        .await?;
    sqlx::query("DELETE FROM delivery_attempts WHERE finished_at<now()-interval '90 days'")
        .execute(&s.db)
        .await?;
...
pub async fn run(s: AppState) {
    ...
    loop {
        interval.tick().await;
        if let Err(e) = tick(&s, owner).await {
            tracing::error!(code = e.code, "worker tick failed");
        }
    }
}
pub async fn tick(s: &AppState, owner: Uuid) -> Result<()> {
    ...
    while let Some(result) = tasks.join_next().await {
        match result {
            ...
            Ok(Err(e)) => {
                failed = true;
                tracing::error!(code = e.code, "delivery database failure");
            }
            Err(_) => {
                failed = true;
                tracing::error!("delivery task stopped");
            }
        }
    }
    if failed {
...
pub async fn deliver(s: &AppState, user: Uuid, id: Uuid, owner: Uuid) -> Result<()> {
    ...
    let (status, response, error, next, invalid) = match &outcome {
        ...
        Err(e) => {
            ...
            (
                if e.retryable && attempt < 5 && next < expires {
                    "retry"
                } else {
                    "dead"
                },
