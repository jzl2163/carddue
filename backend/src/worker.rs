...
async fn planning_tick(s:&AppState) -> Result<()> {
    ...
    for user in users {
        ...
        // Multiple planner instances are safe. A busy user's next tick can catch up.
        ...
        if let Err(e)=planner::reconcile(&mut tx,user,&s.config).await{tracing::error!(user_id=%user,code=e.code,"user planning failed");continue;}
        tx.commit().await?;
    }
...
pub async fn tick(s:&AppState,owner:Uuid) -> Result<()> {
    ...
    for row in rows {
        let id:Uuid=row.try_get("id")?;let user:Uuid=row.try_get("user_id")?;let state=s.clone();
        tasks.spawn(async move{deliver(&state,user,id,owner).await});
    }
    while let Some(result)=tasks.join_next().await {
        match result {Ok(Ok(()))=>(),Ok(Err(e))=>tracing::error!(code=e.code,"delivery database failure"),Err(_)=>tracing::error!("delivery task stopped")}
    }
    Ok(())
}
pub async fn deliver(s:&AppState,user:Uuid,id:Uuid,owner:Uuid) -> Result<()> {
    ...
    if let Some(event)=job.try_get::<Option<Uuid>,_>("event_id")? {
        ...
        let date:chrono::NaiveDate=e.try_get("event_date")?;let tz:chrono_tz::Tz=timezone.parse().map_err(|_|AppError::internal())?;
    ...
    if let Some(rule)=job.try_get::<Option<Uuid>,_>("rule_id")? {
        let enabled:bool=sqlx::query_scalar("SELECT COALESCE((data->>'enabled')::boolean,true) FROM notification_rules WHERE id=$1 AND user_id=$2").bind(rule).bind(user).fetch_one(&mut *tx).await?;allowed &= enabled;
    }
    if !allowed {
        sqlx::query("UPDATE notification_jobs SET status='cancelled',lease_owner=NULL,lease_until=NULL WHERE id=$1").bind(id).execute(&mut *tx).await?;tx.commit().await?;return Ok(());
    }
    let started=Utc::now();let kind:String=c.try_get("kind")?;
    ...
    let outcome=match (rendered,crypto::decrypt(&s.config.encryption_key,&aad(user,connection,&kind),&encrypted)) {
        (Ok(rendered),Ok(bytes)) => {
            ...
            match serde_json::from_slice::<Value>(&bytes) {Ok(config)=>providers::send(&kind,&config,&rendered,id,&s.config).await,Err(_)=>Err(providers::Delivery{status:None,retryable:false,invalid_subscription:false,code:"CREDENTIAL_FORMAT",retry_after:None})}
        }
        (Err(_),_)=>Err(providers::Delivery{status:None,retryable:false,invalid_subscription:false,code:"TEMPLATE_RENDER_FAILED",retry_after:None}),
        (_,Err(_))=>Err(providers::Delivery{status:None,retryable:false,invalid_subscription:false,code:"CREDENTIAL_DECRYPT_FAILED",retry_after:None}),
    };
    ...
    // The user lock is intentionally held across the bounded provider call. A committed
    // mark-paid action cannot race a later send; an already-in-flight send is not recallable.
    // A crash after provider acceptance but before this commit can still cause a duplicate.
    tx.commit().await?;Ok(())
}
