...
async fn event(conn: &mut PgConnection, user: Uuid, card: Uuid, cycle: Option<Uuid>, kind: &str, title: &str, date: NaiveDate, paid: bool, id: Uuid) -> Result<()> {
    sqlx::query("INSERT INTO calendar_events(id,user_id,card_id,cycle_id,kind,title,event_date,paid) VALUES($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT(id) DO UPDATE SET title=EXCLUDED.title,event_date=EXCLUDED.event_date,paid=EXCLUDED.paid,active=true,revision=calendar_events.revision+1,updated_at=now() WHERE (calendar_events.title,calendar_events.event_date,calendar_events.paid,calendar_events.active) IS DISTINCT FROM (EXCLUDED.title,EXCLUDED.event_date,EXCLUDED.paid,true)")
        .bind(id).bind(user).bind(card).bind(cycle).bind(kind).bind(title).bind(date).bind(paid).execute(conn).await?;
    Ok(())
}
/// Caller holds the user's row lock. All regenerated dates, events and pending jobs commit atomically.
pub async fn reconcile(conn: &mut PgConnection, user: Uuid, cfg: &Config) -> Result<()> {
    ...
    for row in rows {
        ...
        for cycle in &mut card_cycles {
            ...
            if cycle.paid_at.is_none() && cycle.cycle_month>=current {
                ...
                if statement!=cycle.statement_date || due!=cycle.due_date {
                    ...
                    cycle.statement_date=statement; cycle.due_date=due;
                }
            }
            for (kind,date) in [("statement",cycle.statement_date),("payment_due",cycle.due_date)] {
                let id = stable(&format!("{}/{kind}",cycle.id));
                ids.push(id);
                event(conn,user,card,Some(cycle.id),kind,dates::label(kind),date,kind=="payment_due" && cycle.paid_at.is_some(),id).await?;
            }
        ...
        if let (Some(m),Some(d)) = (c.annual_fee_month,c.annual_fee_day) {
            for year in start.year()..=end.year() {
                ...
                ids.push(id); event(conn,user,card,None,"annual_fee","年费日",date,false,id).await?;
            }
        }
        cards.insert(card,c);
    }
    ...
    for row in milestones {
        ...
        for (key,date) in occurrences {
            ...
            ids.push(id); event(conn,user,m.card_id,None,&m.kind,&m.title,date,false,id).await?;
        }
    }
    // Retain cancelled events as tombstones, preserving UID and revision for calendar clients.
