use crate::{
    ...
    model::{CardInput, CycleView, Event, MilestoneInput, RuleInput, TemplateInput},
};
...
pub async fn reconcile(conn: &mut PgConnection, user: Uuid, cfg: &Config) -> Result<()> {
    ...
    for row in rows {
        ...
        let mut card_cycles = sqlx::query_as::<_, CycleView>(CYCLES)
            .bind(user)
            .bind(card)
            .fetch_all(&mut *conn)
            .await?;
        for cycle in &mut card_cycles {
            if cycle.cycle_month < start || cycle.cycle_month >= end {
                continue;
            }
            if cycle.paid_at.is_none() && cycle.cycle_month >= current {
                let (statement, _) = dates::cycle_dates(&c, cycle.cycle_month)?;
                let statement = if cycle.statement_overridden {
                    cycle.statement_date
                } else {
                    statement
                };
                let due = if cycle.due_overridden {
                    cycle.due_date
                } else {
                    dates::due_for_statement(&c, cycle.cycle_month, statement)?
                };
                if due < statement {
                    return Err(AppError::bad(
                        "An overridden due date conflicts with the new statement rule",
                    ));
                }
            ...
            for (kind, date) in [
                ("statement", cycle.statement_date),
                ("payment_due", cycle.due_date),
            ] {
                ...
                event(
                    ...
                    EventDraft {
                        ...
                        cycle: Some(cycle.id),
                        kind,
                        title: dates::label(kind),
                        date,
                        paid: kind == "payment_due" && cycle.paid_at.is_some(),
                    },
                )
                .await?;
            }
    ...
    let milestones =
        sqlx::query("SELECT id,data FROM card_milestones WHERE user_id=$1 ORDER BY id")
            .bind(user)
            .fetch_all(&mut *conn)
            .await?;
    ...
    let rules = sqlx::query("SELECT id,data,created_at FROM notification_rules WHERE user_id=$1")
        .bind(user)
        .fetch_all(&mut *conn)
        .await?;
    let connections: Vec<Uuid> = sqlx::query_scalar(
        "SELECT id FROM notification_connections WHERE user_id=$1 AND enabled=true",
    )
    .bind(user)
    .fetch_all(&mut *conn)
    .await?;
    let template_rows = sqlx::query("SELECT id,data FROM notification_templates WHERE user_id=$1")
        .bind(user)
        .fetch_all(&mut *conn)
        .await?;
    let mut templates: HashMap<Uuid, Value> = HashMap::new();
    for row in template_rows {
        templates.insert(row.try_get("id")?, row.try_get("data")?);
    }
    ...
    for row in rules {
        ...
        let Some(template) = templates.get(&rule.template_id) else {
            continue;
        };
        ...
        for e in &events {
            if e.kind != rule.event_kind
                || !rule.card_ids.is_empty() && !rule.card_ids.contains(&e.card_id)
            {
                continue;
            }
            ...
            for offset in &rule.offsets {
                ...
                if when < created || when < now - Duration::hours(24) {
                    continue;
                }
                for connection in &rule.connection_ids {
                    ...
                    if planned > 20000 {
                        return Err(AppError::bad(
                            "Too many scheduled reminders; narrow card, channel or offset selections",
                        ));
                    }
